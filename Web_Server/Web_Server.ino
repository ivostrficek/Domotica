...
const char* ssid = "Plaza Net 2.4GHz";
const char* password = "leiaorgana";
...
void setup(void){
  ...
  webSite +="<meta http-equiv='refresh' content='3'><title>Web Server</title>\n";
  webSite += "<style>body{ background-color: #cccccc; font-family: Arial, Helvetica, Sans-Serif; Color: #000088; }</style>\n";
  webSite +="<h2 dir=\"rtl\" style=\"text-align: left;\"></h2>\n";
 ...
 if (mdns.begin("esp8266", WiFi.localIP())) {
   Serial.println("MDNS responder started");
    ...
    server.on("/", [](){
    String webSiteCopy = webSite;
    signalLevel="";
    signalLevel += "<p><h2>Signal strength: " + dbmToQuality(WiFi.RSSI()) + "</h2></p>\n";
    webSiteCopy+=signalLevel;
    server.send(200, "text/html", webSiteCopy);
    });
  server.on("/socket1On", [](){
    server.send(200, "text/html", "<h1>PUTOOOOOOO</h1>");
